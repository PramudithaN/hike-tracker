...
function analyzeCodebase(rootDir) {
  ...
  const isReactNative = !!allDeps['react-native'] || isExpo
  ...
  if (!description) {
    if (isExpo || isReactNative) description = 'A modern cross-platform mobile application built with React Native, Expo, and TypeScript.'
    else if (isElectron) description = 'A high-performance desktop application built with Electron, React, and TypeScript.'
...
function formatProjectOverview(info) {
  const stack = []
  if (info.isExpo) stack.push('**Expo**')
  ...
  else if (info.isReact && !info.isReactNative) stack.push('**React**')
...
function formatCodingStandards(info) {
  ...
  if (info.hasTs) {
    ...
    if (info.isReactNative) {
      points.push(`   - Ensure smooth cross-platform compatibility across iOS, Android, and Web.\n` +
        `   - Follow Expo Router conventions for navigation, route parameters, and screen layouts.\n` +
        `   - Optimize performance by avoiding unnecessary re-renders in map views, lists, and location subscriptions.`)
    } else if (info.isElectron) {
...
function formatFrameworkRules(info) {
  ...
  if (info.isReactNative) {
    sections.push(`### React Native & Expo Mobile Guidelines:\n` +
      `1. **Background Tasks & Location Tracking:**\n` +
      `   - Handle foreground and background location permissions gracefully with user-friendly prompts.\n` +
      `   - Define background tasks with \`expo-task-manager\` at the root module scope (outside React component renders).\n` +
      `2. **State & Offline Resilience:**\n` +
      `   - Persist critical live state (e.g. active hike coordinates, waypoints) with AsyncStorage / Zustand persist to prevent loss on app kill or background reclaim.\n` +
      `3. **Safe Area & Platform Handling:**\n` +
      `   - Use \`react-native-safe-area-context\` for proper insets across iOS notches and Android status/navigation bars.\n` +
      `   - Use platform stubs or conditional imports for web-safe rendering (e.g. maps).`)
  }
...
function formatSecurityRules(info) {
  ...
  points.push(`4. **Clean Build Artifacts:**\n` +
    `   - Keep build artifacts (\`dist\`, \`dist-electron\`, \`release\`, \`node_modules\`, \`.next\`, \`.expo\`) ignored in \`.gitignore\`.`)
...
### Commit Types:
* \`feat(scope):\` -> A new feature or user-facing capability (e.g. \`feat(tracking): record summit waypoint coordinates\`).
* \`fix(scope):\` -> A bug fix or error correction (e.g. \`fix(auth): handle session refresh on app foreground\`).
* \`perf(scope):\` -> Performance improvement (e.g. \`perf(map): optimize polyline rendering for long tracks\`).
* \`refactor(scope):\` -> Code restructuring without changing functional behavior (e.g. \`refactor(store): extract location slice\`).
* \`security(scope):\` -> Security hardening (e.g. \`security(supabase): add RLS policy for user profiles\`).
* \`style(scope):\` -> Styling, theme, or layout tweaks (e.g. \`style(leaderboard): adjust podium medal badge contrast\`).
* \`docs(scope):\` -> Documentation updates (e.g. \`docs(readme): update environment variable setup\`).
* \`chore(scope):\` -> Maintenance, dependencies, build configuration (e.g. \`chore(deps): bump expo sdk to latest patch\`).
